...
def visualize_network_plotly(edge_trace, node_trace):
    ...
    Returns:
        None
    """
    # Update node_trace marker properties with custom color scale
    node_trace.marker.colorscale = [
        [0, '#0000FF'],      # Blue
        [0.33, '#8A2BE2'],   # Purple
        [0.66, '#FF00FF'],   # Magenta
        [1, '#FF0000']       # Red
    ...
    node_trace.marker.colorbar = dict(
        ...
        bgcolor='#1E1E1E'  # Dark background for color bar
    )

    # Set the font color for actor names (text)
    node_trace.textfont = dict(
        size=10,
        color='white'  # White color for actor names
    )
...
def plot_matplotlib_statistics(degrees_all, avg_successes_all, degrees_fit, avg_successes_fit):
    ...
    # First plot: Degree distribution histogram
    fig_hist = go.Figure()
    ...
    fig_hist.show()
    fig_hist.write_html('html_plots/actor_degree_distribution.html')

    # Second plot: Actor degree vs average success with LOWESS fit
    ...
    fig_scatter.add_trace(go.Scatter(
        ...
        mode='markers', 
        marker=dict(size=8, color=avg_successes_all, colorscale=customColorScale, showscale=True, opacity=0.6),
        name='Actors'
    ))
    ...
    fig_scatter.show()
    fig_scatter.write_html('html_plots/actor_degree_vs_success.html')
